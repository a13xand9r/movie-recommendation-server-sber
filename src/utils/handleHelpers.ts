...
import { fixMovieName, getRandomFromArray } from './utils'
...
export const sendFirstRecommendation = async (
  ...
  const recommendations = await recommendMovies(foundMovies?.results[foundMoviesIndex ?? 0].id)
  ...
  if (recommendations && recommendations?.results?.length > 0) {
    ...
    sendNewMovie(
      ...
      `Рекомендации для фильма ${foundMovies.results[foundMoviesIndex ?? 0].title}. `
    )
  } else {
